...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        src_text = clean_english_text(src_text)
        tgt_text = clean_chinese_text(tgt_text)
        ...
        if enc_num_padding_tokens < 0 or dec_num_padding_tokens < 0:
            # Return a dummy dictionary where everything is padded
            return {
                ...
                "encoder_mask": torch.zeros((1, 1, self.seq_len), dtype=torch.int),
                "decoder_mask": torch.zeros(
                    (1, self.seq_len, self.seq_len), dtype=torch.bool
                ),
                "label": torch.full(
                    (self.seq_len,),
                    self.tokenizer_tgt.token_to_id("[PAD]"),
